...
Воспроизводит LingQ-style виджет, но с шкалой ADR-0005 (`0..5` + known + ignored).

```
[🗑]  [0] [1] [2] [3] [4] [5]  [✓]
```

- **6 пилюль `0..5`** — клик по любой устанавливает `status=tracked` и `confidence=N`. Текущая выделена. На hover — tooltip «Уверенность N/5».
- **`✓` (зелёная)** — клик переводит item в `status=known` (`confidence=NULL`).
- **`🗑` (корзина)** — переводит item в `status=ignored`. Не удаляет запись из БД (это делается через bulk → «Удалить»).

> **Решение по визуалу.** В макете — `1 2 3 4 ✓` (4 уровня + known) + `🗑`. Мы используем `0..5` (6 уровней по ADR-0005). При узком viewport (<lg) пилюли `0..5` сворачиваются в dropdown с числовым select'ом, оставляя только `🗑` и `✓` видимыми.
...
- Confidence picker на mobile — кликабельные точки `0..5` без чисел (число показывается в подписи), `🗑` слева, `✓` справа.
