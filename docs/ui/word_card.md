...
- управлять confidence `0..5` для `tracked`;
...
|---|---|---|
| `new` | ничего (предлагается выбор) | клик `0..5` → `tracked`; клик `✓` → `known`; клик `🗑` → `ignored` |
| `tracked` (`confidence=N`) | пилюля `N` подсвечена | клик другого числа → меняет confidence; `✓` → `known`; `🗑` → `ignored` |
| `known` | `✓` подсвечен | клик `0..5` → revert в `tracked`; `🗑` → `ignored` |
| `ignored` | `🗑` подсвечен | клик `0..5` или `✓` → reactivate; вместо suggestions блок показывает «Ignored. Reactivate?» |
